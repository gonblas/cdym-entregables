...
int main(void)
{
	...
	DDRC &= ~(1 << PORTC0 | 1 << PORTC1);   // Configuro bit0 y bit1 puerto C como entrada
	DDRD = 0xFF;                            // Configuro Puerto D como salida
	PORTC |= (1 << PORTC0 | 1 << PORTC1);   // Habilito Pull-Up en bit0 y bit1 puerto C
...
void seq_1(uint8_t first) {
	PORTD = first ? 0b01010101 : ~PORTD;
	_delay_ms(DELAY);
}

void seq_2(uint8_t first) {
	if(first || PORTD & (1 << PORTD7)) {
		PORTD = 0b00000001;
	}
	else {
		PORTD = (PORTD << 1);
